    ...
    try:
        ...
        def objective(trial):
            ...
            if success and isinstance(metrics_or_err, dict):
                ...
                # 🚨 에러 알림은 최초 1회만 전송하여 스팸 방지
                if not error_notified:
                    send_discord_error(f"전략 수행 실패 (반복 가능성 있음): {str(metrics_or_err)}", pair=symbol, engine=engine)
                    error_notified = True 
        ...
        # 🚨 치명적 오류도 단 한 번만 보고
        if not error_notified:
            send_discord_error(f"워커 치명적 오류: {str(e)}", pair=symbol, engine=engine)
        return {"error": str(e)}
...
def finalize_optuna_study(self, worker_results, study_name: str, data_path: str, engine: str, symbol: str):
    try:
        ...
        if best_trial:
            ...
            # 엑셀 생성 및 파일 저장 로직 유지...
            excel_buf = create_excel_buffer(data, []) # 실제로는 위처럼 데이터 채움
        ...
        send_discord_error(f"최종 집계 중 알 수 없는 오류: {str(e)}", pair=symbol, engine=engine)
