...
_tf_lower = target_tf.lower()
data_htf = data.resample(_tf_lower).agg({
    'Open': 'first', 
    'High': 'max', 
    'Low': 'min', 
    'Close': 'last', 
...
# 🚀 [V9.1] Alignment: 상위 타임프레임 지표를 1분봉 인덱스로 확장 (미래 참조 완벽 차단)
def align_to_1m(htf_arr, source_idx, target_idx):
    s = pd.Series(htf_arr, index=source_idx)
    # 1. 2시간봉 데이터를 1칸(2시간) 뒤로 밀어서, 캔들이 완성된 이후에만 참조되게 함
    s_shifted = s.shift(1) 
    # 2. 밀어낸 과거 데이터를 1분봉 인덱스에 맞춰 빈칸 채우기 (ffill)
    return s_shifted.reindex(target_idx, method='ffill').values
